...
c_ij_path = './dumps/cij_files'
preds_path = './dumps/predictions'
...
with torch.no_grad():
     for batch_idx, (data, lbls) in enumerate(loader):
         ...
         for btch_elem in range(batch_size):
             e = get_entropies(c_ij[btch_elem]).squeeze().detach().cpu().numpy()
             g = get_ginis(c_ij[btch_elem]).squeeze().detach().cpu().numpy()
             m = get_mcLosses(c_ij[btch_elem]).squeeze().detach().cpu().numpy()
             switchFile(torch.max(labels[btch_elem], dim=0)[1].item())[C_IJ].append(c_ij[btch_elem].detach().cpu().numpy())
             switchFile(torch.max(labels[btch_elem], dim=0)[1].item())[GINI].append(g)
             switchFile(torch.max(labels[btch_elem], dim=0)[1].item())[MCLS].append(m)
             switchFile(torch.max(labels[btch_elem], dim=0)[1].item())[ENTR].append(e)
             ...
             #avg_entropies[torch.max(labels[btch_elem], dim=0)[1].item()] += e
             #avg_gini[torch.max(labels[btch_elem], dim=0)[1].item()]      += g
             #avg_mc_loss[torch.max(labels[btch_elem], dim=0)[1].item()]   += m
             
             prim_act_sum[torch.max(labels[btch_elem], dim=0)[1].item()] += 1

prim_act_sum = np.array(prim_act_sum).reshape(num_classes,1)
#print(f"prim_act_sum : {prim_act_sum}\n")
