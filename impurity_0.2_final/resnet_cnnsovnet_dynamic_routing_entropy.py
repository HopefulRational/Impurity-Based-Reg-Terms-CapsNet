...
class ResNetPreCapsule(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        self.conv1 = nn.Conv2d(1, 16, kernel_size=3, stride=1, padding=1, bias=False)#(b_size,16,64,64)
    ...
    def dynamic_routing(self,predictions,ITER=3):
        ...
        for it in range(ITER):
            ...
            if it < ITER - 1: 
               ...
               b_ij = b_ij + delta
        return v_j.squeeze(dim=1), unif_act_wt_entropy(c_ij)
...
class ResnetCnnsovnetDynamicRouting(nn.Module):
    def __init__(self):
        ...
        self.conv_caps1 = ConvCapsule(in_caps=16,in_dim=8,out_caps=16,out_dim=16,kernel_size=3,stride=2,padding=0) # (7,7)
        self.conv_caps2 = ConvCapsule(in_caps=16,in_dim=16,out_caps=32,out_dim=16,kernel_size=3,stride=1,padding=0) # (5,5)
    ...
    def forward(self,x):
        ...
        class_caps, cij_entr4 = self.class_caps(conv_caps3)
        ...
        return class_predictions, (cij_entr1 + cij_entr2 + cij_entr3 + cij_entr4)
